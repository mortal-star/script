...
def load_accounts():
    ...
    accounts = [line.strip() for line in raw_value.replace("\r\n", "\n").split("\n") if line.strip()]
    return accounts
    ...
    for idx, account in enumerate(accounts, 1):
        if "#" not in account:
            print(
                ...
                f"[账号{idx}] 配置格式错误，必须为 Cookie#Authorization"
            )
            failure_count += 1
            continue

        cookie_str, authorization = account.split("#", 1)
        checker = GLaDOSAutoCheckin(parse_cookies(cookie_str), authorization, idx)
